...
csv_file = 'tensor_times_contraction.csv'
check_list = [9]

with open(csv_file, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['n', 'contraction_time'])  # Header row

    # Run the function for each value in check_list and save the results
    for n in check_list:
        tensor_train = create_and_decompose_sin_func_tensor(n)
        tensor_train2 = create_and_decompose_sin_func_tensor(n)
        sum_tt, contraction_time = tt_contract(tensor_train)
        writer.writerow([n, contraction_time])
        print(f"n={n}, contraction_time={contraction_time}")
